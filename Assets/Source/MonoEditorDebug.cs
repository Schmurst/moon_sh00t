...
public class EditorDebugMethod : Attribute
{
...
public abstract class MonoEditorDebug : MonoBehaviour
{
    ...
    static Dictionary<Type, SerialiseParameter> UnitySerialiseFieldByType = new Dictionary<Type, SerialiseParameter> {
        ...
        {typeof(Enum),    (p, i) => {return(object)EditorGUILayout.EnumPopup (i.Name, (Enum)p);}},
    ...
    //----------------------------------------------------------------------
    static object CreateDefaultObject(Type type)
    {
        if (type.IsValueType)
    ...
    //----------------------------------------------------------------------
    static bool CanSerialiseEnumerable(Type _type)
    {
        //array
        if (_type.IsArray)
    ...
    //----------------------------------------------------------------------
    static bool CanSerialiseType(Type _type)
    {
        var param = !_type.IsEnum ? _type : typeof(Enum);
